...
const Carts = () => {
  ...
  const downQty = async (id, detail_id) => {
    try {
      ...
      cart[currentIndex].product_qty -=
        cart[currentIndex].product_qty === 0 ? 0 : 1;
      await AsyncStorage.setItem('cart', JSON.stringify(cart));
      setCart([...cart]);
    } catch (e) {
      console.error(e);
  ...
  const getTotalPrice = () => {
    ...
    if (cart.length === 1)
      return cart[0].product_price_discounted * cart[0].product_qty;
    const totalObj = cart.reduce((prev, curr) => ({
      price:
        curr.product_price_discounted * curr.product_qty +
        prev.product_price_discounted * prev.product_qty,
      qty: 1,
    }));
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({item}) => (
          <View style={styles.wrap}>
            <CartsItem
              item={item}
              deleteItem={() =>
                deleteItem(item.product_id, item.product_detail_id)
              }
        ...
        keyExtractor={item =>
          `${item.product_id} and ${item.product_detail_id}`
        }
        ItemSeparatorComponent={ItemDivider}
        ListFooterComponent={footer}
      />
      <View style={styles.footerStyle}>
        <TouchableOpacity style={styles.footer} onPress={checkOut}>
